...
class SubstrateXSolver:
    ...
    def __init__(self, grid_size=128, domain_size=1e12, dim=2, 
                 tau=1e3, alpha=1e-40, beta=1e-25, gamma=1e-40, chi=1.0):
        """
        Initialize solver with dimensional scaling.
        ...
        alpha : float
            Primary source term coefficient (default: 1e-40)
        beta : float
            Scale switch coefficient (default: 1e-25)
        gamma : float
            Force coupling term (default: 1e-40)
        chi : float
            Advection coherence (dimensionless, default: 1.0)
        """
        # Physical constants
        ...
        # Characteristic scales based on domain and relaxation time
        self.L_char = domain_size       # Use domain size as characteristic length
        self.tau = tau                  # Store relaxation time constant (seconds)
        self.T_char = self.tau * 10     # Use 10x relaxation time as characteristic time
        
        # Store base parameters for reference
        self.alpha_base = alpha  # 1e-40
        self.beta_base = beta    # 1e-25
        self.gamma_base = gamma  # 1e-40
        self.chi_base = chi      # 1.0 (dimensionless)
        
        # Apply dimensional corrections with domain-relative scaling
        # alpha: [s] L / M → divide by (L³/T²)
        self.alpha = alpha / (self.L_char**3 / self.T_char**2)
        
        # beta: [s] L / M → divide by (L² T)
        self.beta = beta / (self.L_char**2 * self.T_char)
        
        # gamma: [s] L³ / M → divide by (L³/T²)
        self.gamma = gamma / (self.L_char**3 / self.T_char**2)
        
        # chi: dimensionless
        self.chi = chi
        
        # Print scaling information
        print("\n=== Substrate X Solver Initialization ===")
        print(f"Domain: {self.domain_size/1e21:.3f} × 10²¹ m")
        print(f"Time Scale: {self.T_char/3.15e7/1e6:.2f} Myr")
        print(f"Grid: {grid_size}×{grid_size} ({dim}D), dx = {self.dx/1e9:.3f} × 10⁹ m")
        print(f"Time Step: {self.dt:.3e} s, CFL = {self.c * self.dt / self.dx:.4f}")
        print("\nDimensional Parameters:")
        print(f"α = {alpha:.3e} → Scaled: {self.alpha:.3e} [s] L/M")
        print(f"β = {beta:.3e} → Scaled: {self.beta:.3e} [s] L/M")
        print(f"γ = {gamma:.3e} → Scaled: {self.gamma:.3e} [s] L³/M")
        print(f"χ = {chi:.3f} (dimensionless)")
    ...
    def rhs(self, s, s_vel):
        ...
        force_term = self.gamma * (self.F[:,:,0]**2 + self.F[:,:,1]**2)  # |F|²
