#!/usr/bin/env python3
"""
Complete Field Theory Solver for Substrate X - FIXED VERSION
...
class CompleteFieldTheorySolver:
        ...
        for i in range(1, self.grid_size-1):
            for j in range(1, self.grid_size-1):
                laplacian[i,j] = (field[i+1,j] + field[i-1,j] + 
                                 ...
                                 4 * field[i,j]) / (self.dx**2)
        return laplacian
    
    def compute_field_evolution(self):
        """Evolve E and F fields according to coupling equations - FIXED"""
        E_new = self.E.copy()
        F_new = self.F.copy()
        
        # Compute laplacians using fixed method
        laplacian_E = self.compute_laplacian(self.E)
        laplacian_F = self.compute_laplacian(self.F)
    ...
    def evolve_substrate(self):
        ...
        # Compute substrate laplacian
        laplacian_rho = self.compute_laplacian(self.rho)
...
def run_complete_field_theory():
    ...
    for pattern in patterns:
        ...
        results, diagnostics = solver.evolve_system(steps=100, pattern=pattern)  # Reduced steps for testing
    ...
    for idx, pattern in enumerate(patterns):
        ...
        # Dynamic range for fields
        E_range = max(0.1, np.max(np.abs(final['E'])))
        F_range = max(0.1, np.max(np.abs(final['F'])))
        
        im2 = axes[idx, 1].imshow(final['E'], extent=[-0.5,0.5,-0.5,0.5],
                                 origin='lower', cmap='RdBu_r', vmin=-E_range, vmax=E_range)
        ...
        im3 = axes[idx, 2].imshow(final['F'], extent=[-0.5,0.5,-0.5,0.5],
                                 origin='lower', cmap='RdBu_r', vmin=-F_range, vmax=F_range)
