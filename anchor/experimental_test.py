...
print(f"with A = 1.58e-09, α = 1.254")
...
4. ANALYSIS:
   • Plot γ/β vs ρ
   • Fit: γ/β = Aρ^α
   • Compare with predicted: A = 1.58e-09, α = 1.254
...
print(f"\nKEY PREDICTION:")
print(f"At 1000 Pa (~0.01 atm): τ ≈ {1/(A * ((1000*M_N2/(R*T))**alpha)):.0f} s")
...
with open('experimental_design.txt', 'w') as f:
    ...
    f.write(f"\nCRITICAL PREDICTION:\n")
    f.write(f"  At 1000 Pa: τ ≈ {1/(A * ((1000*M_N2/(R*T))**alpha)):.0f} s\n")
