#!/usr/bin/env python3
"""Debug WHERE we should measure the F field for k_eff calculation - FIXED"""
import numpy as np
import sys
import os
...
class DebugSolver(SubstrateXSolver):
    def add_point_mass_debug(self, mass, position, k_eff=0.0):
        ...
        if self.dim == 2:
            ...
            print(f"  dx: {self.dx:.1e} m")
            
            # Check immediate neighbors (using dx for both directions)
            for dx, dy in [(0,0), (1,0), (2,0), (3,0)]:
                dist = np.sqrt((dx*self.dx)**2 + (dy*self.dx)**2)  # Use dx for both
