...
class StabilizedSurgicalSolver(CompleteFieldTheorySolver):
    def __init__(self, M_factor=10000.0, eta_power=20.0, rho_cutoff=0.8, dt=1e-6, **kwargs):
        # Override dt before calling parent constructor
        kwargs['dt'] = dt
        ...
        print(f"   Reduced dt: {dt:.1e} (100× stabilization)")
...
def clean_scale_analysis(solver):
    ...
    rho_rms = np.sqrt(np.mean(rho**2))
    
    print(f"F FIELD: max={F_max:.3f}, min={F_min:.3f}, RMS={F_rms:.3f}")
    print(f"ρ FIELD: RMS={rho_rms:.3f}")
    ...
    if len(radial_F) > 5 and abs(radial_F[0]) > 1e-10:
        short_scale = radial_F[0] / radial_F[2] if abs(radial_F[2]) > 1e-10 else 0
        long_scale = radial_F[2] / radial_F[5] if abs(radial_F[5]) > 1e-10 else 0
...
solver = StabilizedSurgicalSolver(
    ...
    eta_power=20.0,
    rho_cutoff=0.8,
    dt=1e-6  # Critical: 100× reduction for stability
)
