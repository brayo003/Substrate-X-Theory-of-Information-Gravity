...
class SubstrateInterference:
    ...
    def __init__(self, grid_size=256, box_size=30.0, m_X=0.1):
        ...
        # Substrate field (complex wavefunction) - initialize as complex
    ...
    def create_wave_packets(self, positions, widths, momenta):
        ...
        for (x0, y0), width, (px, py) in zip(positions, widths, momenta):
            # Gaussian wave packet with momentum - ensure complex type
            packet = np.exp(-((self.XX - x0)**2 + (self.YY - y0)**2) / (2 * width**2))
            # Explicitly cast to complex for the plane wave factor
            phase_factor = np.exp(1j * (px * self.XX + py * self.YY))
            packet = packet.astype(complex) * phase_factor  # Force complex type
            self.psi += packet
    ...
    def substrate_dynamics(self, dt=0.01, steps=1000):
        ...
        for step in range(steps):
            ...
            # Substrate potential term (nonlinear)
            potential = 0.5 * self.m_X**2 * np.abs(self.psi)**2
