...
def analyze_final_state(solver):
    ...
    for i in range(min(11, len(radial_rho))):
        # Prevent division by zero, use a small constant if F(r) is near zero
        F_val = radial_F[i]
...
# Create and run the calibrated solver
solver = CalibratedSolver(
    alpha=1e-5, beta=0.5, gamma=0.2,
    delta1=25.0, delta2=0.3, kappa=0.4,
    tau_rho=0.1, tau_E=0.1, tau_F=0.1,
    grid_size=64, 
...
print("Running 500-step evolution (50-step print interval)...")
solver.evolve_system(500, print_interval=50)
