#!/usr/bin/env python3
"""
ANALYZE WIKIPEDIA CLICKSTREAM - FINAL PARSER FIX
Disables quoting logic to handle non-standard quotes in TSV data.
"""
...
import gzip
import random
import csv # New import required for quoting parameter
...
def load_and_sample_wikipedia_data(sample_size=100000):
    ...
    # CRITICAL FIX: Disable quoting to stop CSV parser from misinterpreting titles with quotes
    try:
        chunk_iterator = pd.read_csv('clickstream-sample.tsv.gz', 
                                     ...
                                     engine='python',
                                     on_bad_lines='warn',
                                     quoting=csv.QUOTE_NONE, # <-- THE FIX
                                     doublequote=False)      # <-- THE COMPLEMENTARY FIX
    ...
    for chunk in chunk_iterator:
        ...
        # Check if the chunk is not empty (can happen near file end)
        if chunk.empty:
            continue
        ...
        for _, row in chunk.iterrows():
            if len(sampled_data) < target_sample:
                ...
                # Use total_rows for probability calculation
                r = random.randint(0, total_rows) 
                if r < target_sample:
                    # Replace a random element in the sampled list
...
def compute_wikipedia_S(df):
    ...
    # Use dropna() and list comprehension for efficiency and safety
    links = internal_links[['prev_title', 'curr_title', 'n']].dropna()
    
    for _, row in links.iterrows():
        # Handle potential KeyErrors if a title was dropped due to NaN but somehow appears here
        try:
...
# MAIN EXECUTION
print("🎯 WIKIPEDIA UTD ANALYSIS - FINAL PARSER FIX")
print("=" * 50)
